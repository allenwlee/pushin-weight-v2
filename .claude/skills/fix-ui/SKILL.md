...
4. Before release handoff, record the product-source revision in the declaration and run `uv run --extra dev python -m tests.ui_assurance.gate --scope candidate --candidate-revision <product-source-sha>`. Require every normalized obligation to pass and the generated Bridgewright assessment to report zero failed, skipped, errored, missing, or unknown obligations.
   Choose performance scope from the actual change risk before running. `--performance-mode candidate` (default) requires measurement of the same product-source revision, with sealed evidence confirming that identity. Use it for changes to rendering cost, request volume, query behavior, assets, or interaction timing. `--performance-mode baseline --performance-rationale <reason>` labels another revision as a comparison only; it cannot satisfy a candidate performance requirement. `--performance-mode not-required --performance-rationale <reason>` records an explicit risk-based omission (for example, copy-only work) while keeping all product obligations required. Record owner waivers as waived, never passed.
   Candidate and baseline measurement modes require explicit Bridgewright performance-run inputs (`--performance-state-home`, `--performance-runtime`, `--performance-target-revision`, `--performance-installed-package-commit`, `--performance-data-source-identity`, `--performance-data-source-kind`, `--performance-declaration`, and `--performance-fixture-digest` for fixture runs); they run the public performance-run and performance-result operations. Candidate measurement requires a clean sealed result for the candidate revision; a baseline result is reported separately.
...
Evidence can be reused when the relevant code, assets, dependencies, fixtures, and environment assumptions are unchanged. Plan or documentation edits alone do not invalidate product evidence. State the preserved source revision and the reason for reuse. Changes to this verification wrapper itself need focused wrapper regressions; they do not require a full application-browser/performance run unless application behavior also changed.
