...
#
# Invoked by: com.fuchitalee.x-monitor.harvest
# (pair: com.fuchitalee.x-monitor.config-reload for WatchPaths triggers).
# Called by the StartCalendarInterval at minute 0,15,30,45 of every hour.
...
# To resume, remove the file. Both this scheduled wrapper and the
# WatchPaths wrapper check the same sentinel so a config edit can't
...
if [[ $RC -ne 0 ]]; then
  ...
  TAIL=$(tail -3 "$LOG" | tr '\n' ' ' | head -c 200)
  osascript -e "display notification \"Exit $RC — $TAIL\" with title \"x-monitor pipeline failed\" subtitle \"harvest run\"" 2>/dev/null || true
else
  ...
  if [[ "$N_DROPS" -gt 20 ]]; then
    osascript -e "display notification \"Exit 0 — $N_DROPS signals dropped (LLM/brand drift?)\" with title \"x-monitor high signal-drop\" subtitle \"harvest run\"" 2>/dev/null || true
  fi
fi
exit $RC
