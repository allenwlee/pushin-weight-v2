# {{AGENT_ATTRIBUTION}}
"""Migration 005 tests: products + hf_orgs tables (company-centric redesign).

Migration 005 adds two additive tables (no existing data touched):
...
These tests verify:
  - 005 applies on a fresh DB (Store auto-migrates 001→005) and both tables
    exist with the expected columns; version 5 is in the ledger.
  - brand_hf_orgs is GONE (the prior design's table does not exist after 005).
  - the curated company→HF-org seed is present (11 rows, all confirmed).
  - re-opening is idempotent (no duplicate seed rows, ledger version 5 once).
  ...
  - 005 does not disturb migration 004's 12-brand seed.
  - 005 adds the missing `minimax` company + brand_companies edge.
"""
...
# --- migration 005: tables + seed --------------------------------------


def test_migration_005_creates_tables_and_columns(tmp_path):
    """Both tables exist with key columns; ledger records version 5."""
...
def test_migration_005_seeds_curated_orgs(tmp_path):
...
def test_migration_005_seeds_minimax_company(tmp_path):
    """Migration 005 also adds the missing `minimax` company + brand_companies edge."""
    from x_monitor.store import Store
...
def test_migration_005_idempotent(tmp_path):
    """Re-opening a DB with 005 applied does not duplicate the seed."""
    from x_monitor.store import Store
...
def test_products_hf_type_defaults_to_model(tmp_path):
    ...
    s = Store(tmp_path / "x.db", auto_migrate=True)
    try:
        # 'deepseek-ai' is already seeded by migration 005 (curated).
        s._conn.execute(
...
# --- migration 005 does not disturb 004 --------------------------------


def test_migration_005_preserves_brands_seed(tmp_path):
    """005 is additive: read_brands() still returns the 12 seeded brands."""
    from x_monitor.store import Store
...
def test_migration_005_company_count(tmp_path):
    """005 adds the minimax company (the 11th); 004's seed had 10."""
    from x_monitor.store import Store
