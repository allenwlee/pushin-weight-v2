"""Migration 007: enum i18n lookup tables + FK conversion.
...
def test_migration_007_creates_signal_keys_and_labels(tmp_path):
...
def test_migration_007_creates_role_keys_and_labels(tmp_path):
...
def test_migration_007_creates_engagement_tier_keys_and_labels(tmp_path):
...
def test_migration_007_post_brand_signals_signal_is_fk(tmp_path):
...
def test_migration_007_post_brand_signals_check_constraint_preserved(tmp_path):
...
def test_migration_007_brand_accounts_role_is_fk(tmp_path):
...
def test_migration_007_company_accounts_role_is_fk(tmp_path):
...
def test_migration_007_accounts_engagement_tier_is_fk(tmp_path):
...
def test_migration_007_full_stack_apply(tmp_path):
    """All migrations 001-007 apply on a fresh DB; data round-trip
    ...
    try:
        ...
        # 001-004, 006, 007 — no 005 (HF products lives on unmerged branch).
        assert applied == [1, 2, 3, 4, 6, 7], f"unexpected versions: {applied}"
    finally:
        s.close()


def test_migration_007_round_trip_post_brand_signals(tmp_path):
