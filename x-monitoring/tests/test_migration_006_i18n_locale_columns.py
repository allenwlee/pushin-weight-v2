"""Migration 006: i18n locale columns on brands / companies / accounts.
...
def test_migration_006_adds_display_name_columns_to_brands(tmp_path):
    """After migration 006, brands has display_name_en and display_name_zh_cn."""
...
def test_migration_006_adds_display_name_columns_to_companies(tmp_path):
    """After migration 006, companies has display_name_en and display_name_zh_cn."""
    from x_monitor.store import Store
...
def test_migration_006_adds_bio_columns_to_accounts(tmp_path):
    """After migration 006, accounts has bio_en and bio_zh_cn."""
    from x_monitor.store import Store
...
def test_migration_006_creates_partial_backfill_indexes(tmp_path):
...
def test_migration_006_idempotent(tmp_path):
    """Re-opening a DB that already has 006 applied does not re-run it."""
    from x_monitor.store import Store
...
def test_migration_006_full_stack_apply(tmp_path):
    """All migrations 001-006 apply cleanly on a fresh DB; partial indexes
...
def test_migration_006_null_locale_columns_default(tmp_path):
