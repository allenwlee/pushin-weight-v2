...
ARMS = {
    "0731": {
        ...
        "reasoning": "disabled", "temperature": 1.0, "top_p": 1.0, "seed": 42,
        "capture_profile": "deepseek_0731",
    },
    "incumbent": {
        # This is an alias. The preflight receipt records its current public
        # identity; the contract must not claim a dated pinned model.
        "model": "deepseek-v4-flash", "base_url": "https://api.deepseek.com/anthropic",
        "provider": "DeepSeek direct", "endpoint": None,
        "reasoning": "provider default", "temperature": None, "top_p": None, "seed": None,
...
class CaptureTextClient:
    ...
    def __init__(self, model_arm: str) -> None:
        self.request_profile = ARMS[model_arm]["capture_profile"]
        self.requests: list[dict[str, Any]] = []
...
def _capture_literal(rows: list[dict[str, Any]], model_arm: str) -> list[dict[str, Any]]:
...
def _capture_synthesis(rows: list[dict[str, Any]], model_arm: str) -> tuple[list[dict[str, Any]], list[str]]:
    client = CaptureJSONClient(model_arm)
    cfg = SimpleNamespace(model=ARMS[model_arm]["model"], max_input_tokens_per_post=4_000, max_output_tokens_per_post=SYNTHESIS_MAX_OUTPUT_TOKENS, timeout_seconds=60)
    capped = []
    for row in rows:
        try:
...
def prepare(input_path: Path, directory: Path, model_arm: str = DEFAULT_ARM) -> dict[str, Any]:
    if directory.exists():
    ...
    literal_requests = _capture_literal(rows, model_arm)
    synthesis_requests, synthesis_input_cap_rejections = _capture_synthesis(rows, model_arm)
    ...
    contract = {
        ...
        "source_hashes": _source_hashes(), "model_arm": model_arm, "route": ARMS[model_arm],
...
def run(directory: Path) -> dict[str, Any]:
    ...
    model_arm = contract.get("model_arm", DEFAULT_ARM)
    if model_arm not in ARMS:
        raise ValueError("frozen model arm invalid")
    preflight = executor.price_preflight(model_arm)
    literal.require_price_preflight(model_arm, preflight)
    key = executor.credential(model_arm)
    try:
    ...
    synth_cfg = SimpleNamespace(model=ARMS[model_arm]["model"], max_input_tokens_per_post=4_000, max_output_tokens_per_post=SYNTHESIS_MAX_OUTPUT_TOKENS, timeout_seconds=60)
...
def main() -> None:
    ...
    prepared.add_argument("input", type=Path); prepared.add_argument("directory", type=Path); prepared.add_argument("--model-arm", choices=tuple(ARMS), default=DEFAULT_ARM)
    runner = commands.add_parser("run")
    runner.add_argument("directory", type=Path)
    args = parser.parse_args()
    result = freeze_input(args.selection, args.output, args.review_rubric) if args.action == "freeze-input" else prepare(args.input, args.directory, args.model_arm) if args.action == "prepare" else run(args.directory)
