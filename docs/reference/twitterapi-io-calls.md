...
**v1.8 reattribute one-time cost (commit `cc02a63`):** the 2026-06-20 backfill of `posts_brands_signals` processed ~2,700 brand-rows at ~42 rows/min on direct Haiku 4.5, taking ~65 minutes wall clock and ~$0.10-0.30. Coverage was 74.4% (2,010/2,700 brand-rows; the rest are correct empty-by-design cases where the LLM judges a body-keyword match is a false positive).
