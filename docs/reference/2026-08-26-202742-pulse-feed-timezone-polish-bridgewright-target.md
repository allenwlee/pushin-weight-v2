...
feed identity instructions in the 2026-08-26 session, plus staging-review
corrections on 2026-08-27
...
| One-day time rows | Keep local and comparison times below the plot at the same 24 fixed hourly positions. Show even-hour `H:00` labels directly one row above the other, retain odd-hour hashes only for local time, draw compact row labels inline at the left, and ensure only the local row draws a baseline. |
...
| Model ordering | Pulse chips and the chart legend use one fixed order: DeepSeek, Qwen, MiniMax, Zhipu GLM, Xiaomi MiMo, Moonshot Kimi, InclusionAI, Mistral, StepFun, Baidu ERNIE, Tencent Hunyuan, Meta Llama, NVIDIA Megatron, ByteDance Doubao, 01.AI Yi, SenseTime SenseChat, LG EXAONE, Kuaishou Kling, Sakana AI, Upstage Solar. |
...
- Do not change the 20-model inventory, filter ordering, or locale behavior,
...
- Prove the 1d rows have 24 pixel-aligned positions, directly stacked even-hour
  labels, local-only odd-hour hashes, inline localized left labels,
  mode-dependent prominence, and one baseline.
...
- Prove pulse chips and the chart legend begin DeepSeek, Qwen, MiniMax and keep
  the same fixed 20-model order after refresh.
