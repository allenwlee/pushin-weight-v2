...
| v1.8 reattribute with `--with-llm` | Re-classifies existing `post_brands` rows via the Anthropic API (direct or proxy); populates `post_brand_signals.signal` per (post, brand). Not part of the live cycle — operator-initiated via `python -m x_monitor reattribute --with-llm --batch-size 50`. One-time cost ~$0.10-0.30 for a full 2,700 brand-row backfill. |
