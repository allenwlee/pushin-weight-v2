...
- KTD34. **Use two small review-shaped classifier judgments plus rare-label adjudication.** The first frozen final candidate proved that one large publication-shaped pass preserves rare-label precision but systematically omits supported secondary types; the two blinded ten-post per-brand reviews recovered those types, while their raw union overcalled personnel changes. Production therefore reuses that measured review shape, unions general types and product labels from two ten-post passes, takes scalar judgments from the first pass, lets either pass declare attributed-brand context missing, and adjudicates only proposed `personnel_changes`/`other` pairs. The internal discovery booleans are prompts for explicit review and are discarded rather than published by classification. Valid posts survive a malformed neighbor so fallback cost is proportional to actual failures. This implements R78 and replaces the earlier twenty-post single-pass topology without changing the taxonomy or quality floors.
...
wire that reached 78.6% exact sets before rare-label filtering. A frozen
120-row runtime-v12 probe tests that exact implementation and narrow
adjudication before another full consumed-development run.
