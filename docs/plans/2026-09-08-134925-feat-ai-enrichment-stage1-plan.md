...
- R78. Replace the blocked single-pass twenty-post classifier with a measured quality path that performs two independent ten-post judgments and merges supported general post types and product labels. Use a separately versioned narrow adjudication only for the small set of post-brand pairs where either judgment proposes `personnel_changes` or `other`; the adjudicator cannot add either rare label to an unproposed pair. Treat `context_missing` as attributed-brand relevance: a bare acknowledgement, bare link or careers-page pointer without a role, keyword collision, or content solely about another entity is missing for that brand even when the text is otherwise classifiable. Parse and retain valid batch rows, retry only missing or invalid rows, preserve atomic per-post publication, keep the explicit DeepSeek model, deadline, strict vocabulary, telemetry, shared repair cap, stable output order, and maximum three concurrent transport calls. Freeze and enforce separate request/token/cost budgets before every development or final evaluation.
...
- KTD34. **Use two small classifier judgments plus rare-label adjudication.** The first frozen final candidate proved that one large pass preserves rare-label precision but systematically omits supported secondary types; the two blinded ten-post reviews recovered those types, while their raw union overcalled personnel changes. Production therefore unions general types and product labels from two ten-post passes, takes scalar judgments from the first pass, lets either pass declare attributed-brand context missing, and adjudicates only proposed `personnel_changes`/`other` pairs. Valid rows survive a malformed neighbor so fallback cost is proportional to actual failures. This implements R78 and replaces the earlier twenty-post single-pass topology without changing the taxonomy or quality floors.
