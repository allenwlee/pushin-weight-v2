...
All schema changes are additive. Existing taxonomy-v2 `events_opportunities` rows keep their original combined meaning and version; they are never silently rewritten into the two new categories. No historical LLM reclassification, live provider search, production activation, or public MCP/API release is authorized by this plan amendment.

Staging completion requires database and harvester regression tests, frozen provider-free evaluation protocols and fixtures, and an exact-SHA staging deployment. A later production proposal requires the paid/adjudicated taxonomy-v2 baseline, the three-stratum taxonomy-v3 classification assessment, separate post-level discovery assessments, and role/affiliation extraction assessments. The largest unresolved risks are real TwitterAPI query behavior and cost, multilingual recall, sparse job fields, and organization identity review; the Grok artifact helps design those tests but is too concentrated and non-exhaustive to prove quality or coverage.

## Goal Capsule

- **Objective:** Readers, operators, and agents can use version-exact Stage 1 classification and structured employment, job, event, and opportunity data across EN, ZH-CN, and JA, including AI organizations beyond the tracked brand catalog, without fabricating semantic equivalence, dates, organizations, or past judgments.
- **Means:** Preserve the completed discourse-free per-brand contract and v1/v2 migration proof, freeze the taxonomy-v2 evaluation protocol and exact prompt identity, then add the thirteen-type v3 contract, additive entity/evidence schema, bounded global job- and personnel-discovery lanes, gated structured extractors, and explicit version/lifecycle analysis policies (KTD1–KTD22). Run the paid/adjudicated v2 and v3 assessments only as separately authorized preproduction gates.
...
- **Execution profile:** U1–U11 and their staging receipts are completed historical releases. U12 establishes the taxonomy-v2 evaluator and freezes the before-change protocol; U12A is the deferred, separately authorized preproduction assessment. U13 freezes taxonomy v3; U14 adds schema; U15, U15A, and U15B build provider-free profile, job-discovery, and personnel-discovery paths; U16 activates targeted extraction; U17 reconciles provider-free evaluation contracts and delivers the exact candidate only to staging.
- **Stop conditions:** Stop if a migration changes original state provenance, touches unversioned legacy edges, double-counts aliases, rewrites v2 combined rows, requires historical model inference, invents dates/application routes/brand authority, bypasses `CycleRunner` safeguards, conflates posts with listings, makes Release A unable to run against Release B data, or makes an analysis result silently blend approximate and exact populations.
...
- R33. Use `PersonBrandAffiliation` / `people_brand_affiliations` with a surrogate primary key and FKs to `people` and `brands`. Store `affiliation_type` (`employment`, `founder`, `advisor`, `board_member`, `contractor`, `ambassador`, `creator_partner`, `affiliate`, `investor`, `community`, `other`), observed organization name and handle, raw and normalized title, department, team, job function, seniority, employment type, status (`current`, `former`, `future`, `unknown`), nullable start/end dates plus precision (`day`, `month`, `year`, `unknown`), location, workplace type, description, confidence, review status, optional source-system/external identifiers, a deterministic claim identity, and created/updated timestamps. Brand is authoritative; company is derived through the brand-company relationship rather than duplicated as an affiliation FK. An internship is employment with an internship employment type; a former employee or intern remains a former employment affiliation rather than becoming community. Multiple legitimate periods or roles for one person and brand remain separate rows; only the same normalized claim identity deduplicates.
...
- R57. Evaluate personnel discovery separately from personnel classification and affiliation extraction. Discovery reports source-post precision and estimated coverage for known-lab, untracked-lab, current, former, future, first-person, official/staff, and corroborated-third-party cases across EN/ZH-CN/JA. Classification reports `personnel_changes` precision/recall, while extraction reports person/organization resolution, relationship type/status, date non-invention, evidence retention, and review routing. Include hard negatives such as static biographies, employee spotlights, job listings, model/team changes without a person, and bare organization mentions. No live provider lane or production activation occurs before its offline query bakeoff, maximum cost report, frozen assessment, and separate owner authorization.
...
- Stage 1 production promotion, U12A's paid/adjudicated taxonomy-v2 baseline, the taxonomy-v3 three-stratum assessment, and their numeric real-label quality decisions.
...
- Candidate/application pipeline management, recruiter write access, automated outreach, compensation history for people, and public MCP/API activation before access, privacy, and data-quality gates are separately approved.
...
U1–U5 and their exact-SHA staging receipt are immutable historical evidence for the first Stage 1 delivery. The follow-up begins at U6. U6 owns the crosswalk, future version constants, and label contract. After U6 commits, U7 owns its migration, seed, reader, view, headline, and health files while U8 may build only its new shared-analysis module, management command, new analysis tests, new reference, and `AGENTS.md` link in parallel. U8 does not edit any existing U7-owned view, static, headline, cache, health, or database-test surface until U7 commits and the parent transfers ownership. U8 completion and all Release A candidate/staging gates remain dependent on U7. Only after Release A passes staging may U9 own activation of taxonomy v2/prompt v3, the edge migration, and the single-owner classifier-prompt exhibit update. U10 updates canonical output consumers against that committed packet. U11 reconciles and verifies after all dependencies; it does not pre-edit U6 or U9 files. U12 supplies the evaluator and freezes the exact v2 prompt and assessment protocol. U12A owns the later paid candidate run, blinded adjudication, floor policy, and durable ten-type R17 assessment as a separately authorized preproduction gate; it does not block U13 implementation or staging. U14 owns additive identity, history, discovery-candidate, job/evidence, personnel-run, event, and opportunity schema. After U14, U15 owns the restartable profile backfill, U15A owns only the disabled job-discovery lane and its offline/cost bakeoff, and U15B owns only the disabled personnel-discovery lane and its independent bakeoff. U16 owns classifier activation and targeted extraction after U13–U15B. U17 owns read projections, provider-free classification/discovery/extraction evaluation contracts, reconciliation, and staging proof. Each unit has one primary owner and routes shared-file conflicts through the next reconciliation unit.
...
- **Verification:** Focused unit and command tests pass under denied provider/network credentials. The runbook separates evaluator readiness from R17 completion: a fresh frozen cohort, independent blinded annotations, adjudication, preregistered support/floors, candidate transport authorization, and the resulting durable assessment remain required before any production proposal.

### U12A. Complete and preserve the taxonomy-v2 R17 assessment (deferred preproduction gate)

- **Goal:** Establish the measured ten-type semantic-quality baseline required before any later production proposal; this separately authorized work does not block Stage 1C implementation or staging.
- **Requirements:** R16, R17, R26; KTD1, KTD5, and KTD12.
- **Dependencies:** U12 evaluator and runbook complete; taxonomy v2/prompt v3 frozen; paid candidate transport and independent adjudication separately authorized under the existing role/config/cost controls.
...
- **Dependencies:** U12 complete with the exact taxonomy-v2 prompt identity, evaluator, and assessment protocol frozen. U12A remains required before any production proposal but does not block this implementation or staging unit.
...
- **Verification:** Provider-denied/fake-client call-chain and regression tests pass; `python -m scripts.harvest_cost` or its extended report shows the maximum per-cycle/day call and credit delta; the calibration report names query hypotheses, known false positives, syntax/capability gaps, and separate post/listing/organization counts. No live provider run or production activation occurs without separate authorization.
...
| Offline evaluation | U1, U5, U12 | Stored fixture/candidate JSON yields 100% contract validity and zero fabricated defaults. The provider-free evaluator rejects non-gold truth, preserves coverage failures, and deterministically reports dimensioned metrics and support gaps. U12 freezes the exact taxonomy-v2 prompt identity and assessment protocol before U13; deferred U12A records the fresh heldout gold assessment before any production proposal. |
...
- U12A (deferred preproduction gate): Before any production proposal, one durable taxonomy-v2 R17 assessment identifies the exact frozen cohort, candidate, gold, reviewer/adjudication, floor policy, and code/model/prompt artifacts; reports coverage, support, metrics, limitations, and a non-blocked decision; and remains the immutable before-change semantic baseline. It is not part of the current staging Definition of Done.
...
- U17: Stable affiliation, `employment_history`, evidence, `job_listings`, event, opportunity, and explicit-`as_of` lifecycle projections plus provider-free evaluation contracts, calibration/cost reports, and exact-SHA staging proof pass. Three-stratum classification, separate job/personnel post-discovery, and role/affiliation-extraction real-label assessments remain preproduction gates; production taxonomy, production job-query, production personnel-query, and public MCP/API activation remain separately authorized.
