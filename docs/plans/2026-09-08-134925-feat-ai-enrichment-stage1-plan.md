...
- KTD34. **Use two small review-shaped classifier judgments plus a narrow audit.** The first frozen final candidate proved that one large publication-shaped pass preserves rare-label precision but systematically omits supported secondary types; the two blinded ten-post per-brand reviews recovered those types, while their raw union overcalled personnel changes. Production therefore reuses that byte-exact measured review prompt, unions general types and product labels from two ten-post passes, takes scalar judgments from the first pass, lets either pass declare attributed-brand context missing, and audits only proposed `personnel_changes`/`other` pairs plus posts with broad unsanctioned-signal candidates. The internal discovery booleans are prompts for explicit review and are discarded rather than published by classification; unsanctioned flags live only in the narrow audit. Valid neighboring posts survive malformed rows so fallback cost is proportional to actual failures, while publication still requires two valid base judgments and any required audit. This implements R78 and replaces the earlier twenty-post single-pass topology without changing the taxonomy or quality floors.
...
flowchart TB
  ...
  D --> C
  C -->|yes| E{Rare label or unsanctioned candidate?}
  E -->|yes| F[Narrow audit]
  E -->|no| G[Atomic current-version publication]
  F -->|valid| G
  F -->|invalid| H[Leave pending]
  G --> I[Signal rows and product-label edges]
  G --> J[Per-brand outcome version and nationalism]
  I --> K[Feed charts and headline facts]
  J --> K
```
...
frozen 120-row budget must pass before another full consumed-development run;
this probe still cannot approve release.
