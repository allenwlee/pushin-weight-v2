...
- R78. Replace the blocked single-pass twenty-post classifier with a measured quality path that performs two independent ten-post judgments in the byte-exact candidate-blind per-brand review shape. Exact classified agreements survive; differing classified judgments receive a separately versioned candidate-blind consensus pass that rereads the source and returns one complete judgment without automatic union or averaging. Either base pass may still declare attributed-brand `context_missing`, which skips consensus and remains authoritative. Keep the explicit job/personnel discovery checks in the internal wire to force rare-signal consideration and discard those checks at the classification boundary. Isolate allowlisted unsanctioned flags in a separately versioned narrow audit so flag duties cannot reduce base classification recall; select that audit for posts where any completed judgment proposes `personnel_changes` or `other` or a broad lexical screen finds possible marketing, scam, crypto, or unauthorized evidence. The auditor cannot add either rare label to an unproposed pair, and an audit-required post cannot publish if the audit remains invalid. Treat `context_missing` as attributed-brand relevance: a keyword collision, content solely about another entity, or a bare acknowledgement or link whose meaning or brand relationship depends on absent content is missing for that brand; a concrete brand-attributed source that merely fails the job-listing definition may still support another type or `other`. Parse and retain valid batch rows, retry only posts or consensus pairs containing missing or invalid rows, require two valid base judgments and every required consensus before publication, preserve atomic per-post publication, keep the explicit DeepSeek model, deadline, strict vocabulary, telemetry, shared repair cap, stable output order, and maximum three concurrent transport calls. Freeze and enforce separate request/token/cost budgets before every development or final evaluation.
...
- KTD34. **Use two small reviews, disagreement consensus, and a narrow audit.** The first frozen final candidate proved that one large publication-shaped pass preserves rare-label precision but systematically omits supported secondary types. Two blinded ten-post reviews recover different evidence, but runtime v13 proved that automatic union remains batch-sensitive and cannot meet the exact-set floor. Production therefore reuses the byte-exact measured review prompt, accepts exact classified agreements, and sends only differing classified judgments through the measured candidate-blind consensus prompt. The first pass supplies scalars, either base pass may declare attributed-brand context missing, and the narrow audit handles proposed `personnel_changes`/`other` plus broad unsanctioned-signal candidates. The internal discovery booleans are discarded rather than published; unsanctioned flags live only in the narrow audit. Valid neighboring rows survive malformed rows so fallback cost is proportional to actual failures, while publication still requires two valid base judgments, every required consensus, and any required audit. This implements R78 and replaces the earlier twenty-post single-pass topology without changing the taxonomy or quality floors.
