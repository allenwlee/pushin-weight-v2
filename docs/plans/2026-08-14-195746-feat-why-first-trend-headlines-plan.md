...
- **KTD12 — Every core evaluation headline has quantitative color.** (2026-08-15 user correction.) Low synthetic data quality means sufficient near-threshold coverage, not comparison suppression. Every core packet must expose percentage-change facts for each candidate, and every headline claim must cite and display at least one selected-candidate fact after the content-derived why. The quiet sentinel is a 0.1% leader over a flat comparison. True comparison suppression remains a separate deterministic fail-closed regression.
