...
- Plan: `/Users/fuchitalee/development/pushin-weight-v2/docs/plans/2026-08-27-073628-main-plan-local-main-copy.md`
...
3. Delivery stops after exact-SHA staging verification and PR/CI handoff.
   Production deployment, production Blueprint application, and production
   service suspension or rescheduling are not authorized.
...
staging while production posts continue to arrive on schedule. It must not
require suspending, rescheduling, or redeploying production.
...
- **PD3 — Staging may call providers (user-directed).** Calls are real but
  ...
  release.
- **PD4 — Delivery target is staging (user-approved).** Implement, test,
  deploy, and verify staging; open the PR and observe CI; do not promote to
  production.
...
- **R1 — No production interruption.** No implementation or delivery step may
  suspend, restart, reschedule, or apply a Blueprint to production.
...
- **R11 — Reviewable delivery.** The change is committed to an isolated feature
  branch, staged through the authorized staging path, and handed off as a PR
  with passing local checks and CI. It is not merged or promoted to production
  in this LFG run.
...
- Production deployment or any production service mutation.
...
- `docs/operations/staging-data-refresh.md`
- `docs/operations/2026-08-27-HHMMSS-staging-harvester-acceptance.md` (new,
  timestamp resolved when written)
...
### U5 — Staging delivery and exact-SHA verification
...
7. Open/update the PR, observe CI to a terminal state, resolve actionable
   feedback, and stop. Do not promote or merge to production.
...
- If all gates pass, retain the redacted evidence, open/update the PR, observe
  CI, and stop at staging without production promotion.
...
- A PR is open with terminal CI status and staging evidence.
- No production deployment, merge, or promotion occurred.
