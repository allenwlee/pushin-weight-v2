...
Resume in this same authoritative checkout and branch. Keep failed evidence frozen and use the provisionally widened v3 query. Use the new direct-TypeSafe route, not OpenRouter, and run its independently evaluated, durably captured assessment before enablement once the direct credential is located. The owner authorized separate 57,600-credit first-pass and 300,000-credit continuation ceilings for the ON_DEMAND volume probe on 2026-09-24, removed all percentage-based quality bars, and approved the 28,800-credit recurring reservation cap; none of these authorizes scheduled production collection. The measured volume/budget tradeoff, all remaining implementation units, final review, and exact-SHA staging proof remain. Production is unauthorized.
