...
Resume in this same authoritative checkout and branch. Keep failed evidence frozen and use the provisionally widened v3 query. Use the new direct-TypeSafe route, not OpenRouter. The direct credential is now in this checkout's ignored `.env`; the checked running Render web/headline-worker environments did not have `TYPESAFE_API_KEY`, so staging configuration remains separate. The frozen provider capture and in-sample route correction do not complete R17: the separate live sample has two terminal HTTP 529 receipts and lacks independent labels. The owner authorized separate 57,600-credit first-pass and 300,000-credit continuation ceilings for the ON_DEMAND volume probe on 2026-09-24, removed all percentage-based quality bars, and approved the 28,800-credit recurring reservation cap; none of these authorizes scheduled production collection. The measured volume/budget tradeoff, remaining implementation units, final review, and exact-SHA staging proof remain. Production is unauthorized.
