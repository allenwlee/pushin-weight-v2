...
This is the full feature plan, including durable storage, unknown-name YAML exports, bounded Hugging Face corroboration, a small owner-only approval inbox, failure recovery, tests, and staging delivery. The existing trial command is only an early part of it. A general Hugging Face activity crawler, automatic promotion into paid brand harvesting, and production activation remain out of scope. Staging stays manually triggered; this plan does not add another scheduler.
...
| U3 | Threshold-free gate committed, `b20e1af`, `446eb20`; historical assessment incomplete | 53 focused tests passed, 28 required PostgreSQL tests executed, no skips. Historical live query had 5/20 keepers, but only 37/56 Jev responses were retained; no full-corpus claim or percentage acceptance bar remains. |
...
| U6, U8–U9, U11–U16 | Not implemented | No scheduled lane, canonical-record pipeline, Product verification/inbox, bio integration, end-to-end reader/recovery proof, or staging delivery |
...
Resume in this same authoritative checkout and branch. Keep failed evidence frozen and use the provisionally widened v3 query. Separately correct Jev question/routing design and use a newly versioned, independently evaluated, durably captured assessment before enablement. The owner authorized a separate 57,600-credit first-pass ceiling and 300,000-credit continuation ceiling for the ON_DEMAND volume probe on 2026-09-24, and removed all percentage-based quality bars; these do not authorize a recurring cap increase or scheduled production collection. The measured volume/budget tradeoff, all remaining implementation units, final review, and exact-SHA staging proof remain. Production is unauthorized.
...
- KTD7. **Reserve spend before making a paid call.** Initial limits: 300 Twitter credits per search attempt, 6,000 per UTC day for this lane, one attempt per 15-minute slot. Reserve the full page under a database lock and settle against provider-observed volume; keep the reservation charged when usage is unknown. Empty successful pages use the documented 15-credit floor (one billable result, not 15 tweets). Count raw paid results, including filtered and duplicate results, separately from normalized rows. Trial quality sampling has its own explicit 1,200-credit ceiling; the first-gate volume measurement has a separate owner-authorized 57,600-credit maximum across two 24-hour samples (2026-09-24). These are protective defaults, not an assertion of provider invoices; revalidate pricing before live execution. Revisit the 6,000-credit scheduled daily cap with the owner if measured volume makes continuous 15-minute coverage infeasible; do not silently raise it.
...
- Default production behavior remains unchanged, unused query packs remain off, and exact-candidate staging verification is complete. No production activation, candidate-to-tracked promotion, or general Hugging Face crawler is included; R33's known untracked identities do not enter the scheduled harvester.
