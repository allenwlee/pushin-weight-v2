...
- **KTD8 — Pin a headline route and controls independently.** Add headline-specific base URL, exact dated model, timeout, prompt version, publication epoch, cadences, thresholds, call cap, and three independent default-off controls for serving, enqueueing, and provider calls. The worker checks a shared fail-closed provider-call control/release revision before every reservation so accepted messages cannot spend after shutdown. The initial evaluated default is direct Anthropic `claude-haiku-4-5-20251001`; MiniMax requires its current `MiniMax-M3` route and a separate evaluation.
