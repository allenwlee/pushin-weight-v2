...
- [x] U8. **Replace TEXT primary keys with INTEGER primary keys across all tables** (FULL scope delivered 2026-06-25 via migration 020 — converts all 13 TEXT-PK tables to INTEGER PKs per user authorization on 2026-06-25; INTEGER AUTOINCREMENT id PK + UNIQUE key on each table's natural slug column (e.g. brands.brand_id, accounts.handle, posts.tweet_id, hf_orgs.namespace); FK columns in child tables (posts_brands.brand_id, brands_accounts.author_id, posts.id, etc.) converted to INTEGER-storing-id, backfilled via JOIN against parent.id; 24 tests pass in test_migration_020_text_to_integer_pks.py; the previously-narrowed U8 (commit 4cd62d2) covered only signals + roles enum tables and is now superseded)
...
- [x] U9. **Implement post_types + sentiments taxonomy (per the existing plan)** — DELIVERED 2026-06-25 via migration 022 (originally delivered narrower in commit 4cd62d2 as ADDITIVE columns, unauthorized narrowing flagged by user on 2026-06-25). Remediation: migration 022 drops signal_id + signals + signal_labels tables + makes post_type + sentiment INTEGER NOT NULL FK to `post_type_keys` / `sentiment_keys`; rewrites store.py / intent_classifier.py / attribution.py / reattribute.py / treemap.py / dashboard.py / query_plan.py / run.py; updates trend-chart.js / dashboard.css / _model_card.html.j2; drops `expected_signal` from data/queries/*.yaml + config.yaml::call_c_specs; updates 7+ test files; rewrites attribution.py::build_signal_prompt to ask for (post_type, sentiment) instead of the 6-signal vocabulary.
...
**Original execution (commit 4cd62d2) was UNACCEPTED** because it was additive (kept `signal_id`, kept `signals` table) rather than replacement. The user explicitly authorized full replacement on 2026-06-25. The remediation (migration 022) shipped on 2026-06-25 with the full plan-body scope delivered: 11-file consumer rewrite complete, `expected_signal` removed from all 21 yaml + config, classifier prompt rewritten for `(post_type, sentiment)` vocabulary.
...
**Files:**
- Create: `x_monitor/migrations/022_kill_signal_id.sql` (the kill-switch migration that drops `signal_id` from `posts_brands_signals`, drops `signals` + `signal_labels` tables, and makes `post_type` + `sentiment` NOT NULL — DELIVERED 2026-06-25)
...
- **Interaction graph:**
  ...
  - Dashboard / treemap filters reference `role` column (U6); U9 (DELIVERED 2026-06-25 via migration 022) switches treemap grouping from `signal` to `post_type`+`sentiment`.
...
| Post_types backfill (U9) maps legacy signals incorrectly | Med | Med | **MITIGATED 2026-06-25** — U9 delivered via migration 022 with conservative backfill (`'hands_on_usage'` / `'neutral'` as fallback for any row missing post_type/sentiment). Review queue not needed in practice — backfill handled by the migration's defensive UPDATE. |
...
- **Phase 5 (U9):** post_types/sentiments. **DELIVERED 2026-06-25** via migration 022. Originally delivered in commit 4cd62d2 as migration 019 (additive, not replacement) — the agent narrowed scope without user authorization, keeping `signal_id` / `signals` table alive. User explicitly authorized full replacement on 2026-06-25. Remediation: migration 022 drops `signals` + `signal_labels` + `posts_brands_signals.signal_id` and makes `post_type_id` + `sentiment_id` NOT NULL; rewrites 11 consumer files (4 Python + 1 JS + 1 CSS + 1 Jinja + 7 yaml) + 7+ test files. Depends on U1, U4, U8 (all delivered).
...
## Verification Summary (2026-06-25 — U8 + U9 full scope DELIVERED)

U8 verified at the commit delivering migration 020 (full plan-body scope delivered per user authorization on 2026-06-25). U9 verified at the commit delivering migration 022 (full plan-body scope delivered per user authorization on 2026-06-25).
...
- **U9 migration test sweep:** `test_migration_022_kill_signal_id.py` covers the defensive backfill (`hands_on_usage` / `neutral` fallback), the table rebuild without `signal_id`, the drop of `signals` + `signal_labels`, and the 2 index recreations. Full test suite green.
- **U9 consumer test sweep:** `test_dashboard_i18n`, `test_store`, `test_run`, `test_migration_019`, `test_migration_020`, `test_migration_022` all pass against the post-022 reality (no `signal_id` references anywhere in `x_monitor/`, `data/queries/`, or `tests/`).
- **U9 code drift check:** `grep -r "signal_id\|classify_signal\|expected_signal" x_monitor/ data/queries/ tests/` returns 0 results. `_pick_enum_label` rejects the `signal` family (verified — only supports `'role'`, `'post_type'`, `'sentiment'`). `attribution.py::build_signal_prompt` asks for `(post_type, sentiment)` only.

**Why a verification summary for a delivered plan?** Because both U8 and U9 were first shipped narrower than the plan body promised, and the verification captures that the cut was caught, authorized, and remediated to full plan-body scope on the same day (2026-06-25). Per the project memory `feedback_no_unauthorized_scope_narrowing.md`, an agent that ships narrower scope than the plan body promised must update the plan body to reflect the actual delivery — this section is that update for the U8 + U9 remediation pair.
