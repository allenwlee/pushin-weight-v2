...
**Iter 1 (2026-08-08) v22-master Element Audit surfaced 5 P0 gaps.** Locale default turned out to be a false positive (Django `LANGUAGE_CODE="zh-hans"` already sets zh_cn when no `?locale=` param); the row was removed. Trending %change deltas was **resolved in iter 2** by extending `_build_brands_context()` with `_compute_brand_deltas()`. 3 P0 gaps remain: Top Voices, Feed engagement counts, Feed avatar circles. See `docs/iterations/2026-08-08-v22-iter-001/REPORT.md` + `docs/iterations/2026-08-08-v22-iter-002/REPORT.md`.
