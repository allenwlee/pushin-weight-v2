---
created_at: 2026-09-16T15:11:13+09:00
updated_at: 2026-09-18T20:44:22+09:00
status: adopted-runtime-prompts
...
# Adopted U18A Two-Role Classifier Prompts

These are the complete system-prompt templates used by the two independent classifier calls. Runtime code replaces `{{DECISION_SLOT_KEYS}}` and `{{POST_SLOT_KEYS}}` with deterministic ordered slots. Source evidence is sent separately in the compact user-message envelope below.

The selected runtime uses direct DeepInfra, 20-source batches, temperature `1.0`, top-p `1.0`, seed `42`, reasoning disabled, and application-side JSON validation. Before strict validation, it applies only the observed representation-only normalization: an exact-length positional decision list is mapped to its ordered fixed slots, a valid singleton promotion string becomes a one-item array, and an `outcome` equal to one of its supplied post types becomes `classified`. A content-role `context_missing` decision makes every other axis unavailable during the code-only merge. No missing value or semantic label is inferred. It does not use OpenRouter, native JSON mode, a semantic repair call, or a third classifier call.

## Prompt 1 — Content Classification
...
OUTCOME: classified needs at least one supported brand-specific post type. context_missing means supplied evidence cannot support a brand-specific judgment; use post_types=[] and audience_topics=["unavailable"]. other is a confident residual and is exclusive.
...
## Runtime User Message Shape

Both calls receive the same compact JSON evidence envelope. The values below are placeholders; the keys and nesting match the runtime contract.
...
```json
{
  ...
  "cases": {
    "P01": {
      "brand_decision_slots": {
        "D01": "<brand key>"
      },
      "evidence": {
        "created_at": "<source timestamp>",
        "source_language": "<language>",
        "source_text": "<verbatim post>",
        "english_translation": "<validated stored English translation or empty>",
        "context": [
          "<stored context packet>"
        ],
        "author_affiliations": [
          "<reviewed affiliation facts>"
        ]
      }
    }
  }
...
The executable prompt strings live in `x_monitor/classifier_0731_prompts.py`; the fixed-slot payload, validation, deterministic merge, and fingerprint live in `x_monitor/attribution.py`. This exhibit must be regenerated when either executable prompt changes.
