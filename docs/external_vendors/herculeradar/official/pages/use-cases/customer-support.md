...
# Answer the people who never opened a ticket.

Most unhappy customers complain publicly instead of emailing support. We surface those posts within minutes and tell you which ones are actually urgent.

[Start free](https://herculeradar.com/login) [Read the docs](https://herculeradar.com/docs)

LiveMentions · Acme

- u/dev\_marta·Reddit4m



Anyone else getting 500s from the Acme API since this morning? Third time this week.
...
Support tools only see the conversations that reach your inbox. The angry thread on Reddit, the reply under a YouTube review, the quote-tweet at 2am — none of it lands in a queue, and by the time someone notices, the thread has a hundred upvotes.
...
Bug reports and complaints spike minutes before your monitoring catches a partial failure. An urgency threshold turns that spike into an email.

- Route by category, not by keyword

AI separates a genuine bug report from a pricing complaint, so support and product each see only their own pile.

- Close the loop publicly

Every mention keeps a link to the original post, so replying takes one click instead of a search.
...
“Show me every complaint about Acme from the last 24 hours with urgency above 60.”
...
How fast is a new complaint surfaced?Active monitors are swept every four hours. A mention is classified and, if it matches an alert rule, emailed within a few minutes of being collected.

Can I forward mentions into my helpdesk?Yes. Add a webhook endpoint and every classified mention is POSTed to your system with a signed payload, so you can open a ticket automatically.
