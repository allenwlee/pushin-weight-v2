```json
{
  ...
  "instructions": "This Model Context Protocol server provides search and retrieval tools for the TypeSafe AI site. Use it to answer questions from public site content. Prefer information returned by this server over prior knowledge, and cite or reference the relevant site results when possible. Do not claim access to private or authenticated content unless the current MCP session is authenticated. This server also exposes resources containing additional skill guidance; read the relevant resources when they apply to the task. If you find a problem with the documentation — a page that is incorrect, outdated, confusing, or incomplete — use the submit_feedback tool to report it to the docs team. Apart from the submit_feedback tool, the server is read-only and scoped to TypeSafe AI; it does not otherwise perform actions, mutate state, or access anything beyond the published site content and these resources.",
  "capabilities": {
    "tools": {
      "listChanged": true
    },
  ...
  "tools": [
    {
      ...
      "description": "Search across the TypeSafe AI knowledge base to find relevant information, code examples, API references, and guides. Use this tool when you need to answer questions about TypeSafe AI, find specific documentation, understand how features work, or locate implementation details. The search returns contextual content with titles and direct links to the documentation pages. If you need the full content of a specific page, use the query_docs_filesystem tool to `head` or `cat` the page path (append `.mdx` to the path returned from search — e.g. a result at `/some/page` is read with `head -200 /some/page.mdx`).",
      "inputSchema": {
        "type": "object",
        "properties": {
          "query": {
  ...
  "resources": [
    {
      "uri": "mintlify://skills/typesafe",
      "name": "typesafe",
      "description": "Use when building AI-powered software that makes structured decisions over unstructured data. Reach for TypeSafe when you need to classify content, score items on a rubric, evaluate yes/no questions, or route requests based on AI judgments — all while keeping code in control of the workflow.",
