...
def _capture_literal(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    client = CaptureTextClient()
    cfg = SimpleNamespace(llm=SimpleNamespace(translator_model=executor.TARGET_MODEL, translator_base_url="https://openrouter.ai/api/v1"))
...
def _capture_synthesis(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[str]]:
    client = CaptureJSONClient()
    cfg = SimpleNamespace(model=executor.TARGET_MODEL, max_input_tokens_per_post=4_000, max_output_tokens_per_post=SYNTHESIS_MAX_OUTPUT_TOKENS, timeout_seconds=60)
    capped = []
    for row in rows:
        try:
...
def prepare(input_path: Path, directory: Path) -> dict[str, Any]:
    if directory.exists():
    ...
    literal_requests = _capture_literal(rows)
    synthesis_requests, synthesis_input_cap_rejections = _capture_synthesis(rows)
    ...
    contract = {
        ...
        "source_hashes": _source_hashes(), "route": {"model": executor.TARGET_MODEL, "provider": "DeepInfra", "endpoint": executor.TARGET_ENDPOINT, "reasoning": "disabled", "temperature": 1.0, "top_p": 1.0, "seed": 42},
...
def run(directory: Path) -> dict[str, Any]:
    ...
    preflight = executor.price_preflight(TARGET_ARM)
    literal.require_price_preflight(TARGET_ARM, preflight)
    key = executor.credential(TARGET_ARM)
    try:
    ...
    synth_cfg = SimpleNamespace(model=executor.TARGET_MODEL, max_input_tokens_per_post=4_000, max_output_tokens_per_post=SYNTHESIS_MAX_OUTPUT_TOKENS, timeout_seconds=60)
...
def main() -> None:
    ...
    result = freeze_input(args.selection, args.output, args.review_rubric) if args.action == "freeze-input" else prepare(args.input, args.directory) if args.action == "prepare" else run(args.directory)
