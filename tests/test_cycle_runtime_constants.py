...
- Config.cycle default values match the prior hardcoded values
  (BEFORE: _CURSOR_OVERLAP=60s, _MAX_LOOKBACK=15m, _C1_MAX_RESULTS=150,
...
EXPECTED_CYCLE_DEFAULTS = {
    ...
    "max_lookback_hours": 0.25,          # bounded to one 15-minute slot
...
def test_cycle_runner_uses_config_cycle_not_module_constants():
    ...
    assert runner.cfg.cycle.max_lookback_hours == 0.25
...
def test_seven_call_shape_preserved_post_refactor():
    ...
    cfg = load_config(CONFIG_PATH)
    CycleRunner(dry_run=True, cfg=cfg)
